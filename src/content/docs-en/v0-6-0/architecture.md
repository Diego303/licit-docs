...
4. **Direct imports**: All Phase 1-6 modules use direct imports. Only connectors (Phase 7) use lazy imports.

5. **Automatic detection**: `ProjectDetector` infers languages, frameworks, CI/CD, security tools, and AI agent configurations without the need for manual configuration.
...
## Implemented Modules (Phases 1-6)
...
- **`heuristics.py`**: Engine with 6 heuristics for detecting AI commits (author, message, bulk, co-author, file patterns, time). Weighted average of only signaling heuristics. Supports custom patterns from JSON.
...
10 commands registered with Click, all functional. `report` generates reports in 3 formats (Markdown, JSON, HTML). `gaps` shows gaps with recommendations and suggested tools. `verify` evaluates EU AI Act + OWASP Agentic Top 10 and returns exit codes for CI/CD.
