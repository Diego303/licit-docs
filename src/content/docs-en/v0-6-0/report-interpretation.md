...
| 0-19% | Minimal compliance — typical for a newly initialized project | Run `licit fria`, `licit annex-iv`, `licit trace` |
...
The 11 evaluated articles cover **deployer** obligations (the entity using the AI system), not the provider (the entity that builds it):
...
The 10 risks assess the **security posture** for AI agents:
...
3. **`[!]` (partial) after** — they improve the rate but do not block the pipeline
