...
Run `licit init` at the beginning of the project, not after the fact. The sooner you start tracking, the more complete your compliance evidence will be.
...
- **Protected files**: Files that the agent should not modify.
- **Blocked commands**: Commands that the agent should not execute.
...
Do not wait for the audit. Generate reports on every release:
...
- Ensuring that gaps are prioritized and closed.
...
|---|---|---|
| Ignoring `licit verify` warnings | Partial findings accumulate | Treat partials as technical debt |
| Not version-controlling `.licit.yaml` | Each dev uses a different config | Commit to the repo |
| Pushing `provenance.jsonl` to a public repo | Exposes contributor info | Add to `.gitignore` |
| Generating reports only before audits | Incomplete evidence | Generate on every release |
| Disabling signing "because it's slow" | Loss of verifiable integrity | Sign at least in CI |
| Not updating the FRIA | An outdated FRIA has no value | Update with every significant change |
