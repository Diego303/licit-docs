...
class ComplianceStatus(StrEnum):
    ...
    NOT_APPLICABLE = "n/a"            # Requirement not applicable to the project
...
class ComplianceSummary:
    ...
    compliant: int              # Met
    partial: int                # Partially met
    non_compliant: int          # Not met
...
@dataclass
class CommitInfo:
    sha: str                     # Full SHA hash of the commit
    ...
    co_authors: list[str] = []   # Co-authors (extracted from the body)
```
...
**Final score calculation**: Only heuristics with score > 0 (signaling ones) are averaged:
```python
...
Complete evidence collection for the project (18 fields). Used by evaluators to determine compliance.
