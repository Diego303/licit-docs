...
3. **Defaults**: If no file exists, default values are used
...
- `session-log`: **(Implemented)** Reads AI agent session logs. Currently supports Claude Code (JSONL files in `~/.claude/projects/`). Extensible via Protocol `SessionReader`.
...
Patterns with `*` are resolved using `Path.glob()`. Exact names check existence in git history.
...
**Diff formats:** YAML and JSON produce field-level diffs (`model`, `llm.provider`). Markdown produces section-level diffs (`section:Rules`). Plain text produces a full content diff.
...
changelog:
  watch_files:
    - CLAUDE.md
    - .cursorrules
    - mi-agente-custom.yaml       # additional file
