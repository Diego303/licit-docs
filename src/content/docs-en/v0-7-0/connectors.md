...
licit connectors are read-only integrations with external tools that enrich compliance evidence. They are **optional** — licit works fully without them — but when enabled, they provide data that improves the evaluation.
...
│  │  _run_connectors │──┬── enabled ──→ Formal Connector  │
│  └─────────────────┘  │                                  │
│                        └── disabled ──→ Temporary inline  │
...
[architect](https://github.com/Diego303/architect) is an autonomous coding agent that produces execution reports, audit logs, and a config YAML with guardrails and quality controls.
...
All fields are optional. An empty `{}` is valid and counts as one audit entry.
...
| Article | Evidence from architect | Evidence from vigil |
|---|---|---|
...
| Risk | Evidence from architect | Evidence from vigil |
|---|---|---|
...
The connector must:
1. Implement the full `Connector` Protocol
