...
> Supported version: **0.7.x**. For the formal security policy and vulnerability reporting, see [SECURITY.md](../SECURITY.md).
...
| Code execution via configs | Low | Config content is never executed; only data is parsed |
...
- Odd number of records: the last one is duplicated to complete the pair
...
- Explicit `check=False` — on all `subprocess.run` calls in provenance and changelog (does not raise exceptions on returncode != 0).
...
See [SECURITY.md](../SECURITY.md) in the project root for the complete policy.
