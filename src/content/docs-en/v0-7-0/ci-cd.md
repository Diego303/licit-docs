...
If the pipeline runs on a project that does not have a `.licit.yaml`, licit uses defaults (both frameworks enabled). This works without `licit init`:
