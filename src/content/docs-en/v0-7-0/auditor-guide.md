...
- **1**: There are unmet requirements (see details)
...
|---|---|---|
| **compliant** | Sufficient evidence of compliance | Document as met |
...
| **not-evaluated** | No automated evaluation available | Requires manual evaluation |
...
| "FRIA completed: .licit/fria-data.json" | The FRIA exists and contains data |

---

## EU AI Act — Evaluated articles

licit evaluates the obligations of the **deployer** (the entity deploying the AI system), not the provider:

| Article | What licit verifies | Evidence sought |
|---|---|---|
| Art. 9 — Risk management | Are there guardrails, quality gates, budget limits, security scanning? | Architect configs, vigil, semgrep, snyk |
...
|---|---|---|
| Art. 27 (FRIA) | `licit fria` | Complete the 5-step interactive questionnaire |
| Annex IV | `licit annex-iv` | Run it — automatically generated from project metadata |
...
The signing key is stored in `.licit/.signing-key` and **must not be versioned** in git.

### Append-only data

The provenance store (`.licit/provenance.jsonl`) is append-only by design:
- New runs of `licit trace` add records, never delete
- For a given file, the most recent record prevails
- The full history remains as an audit trail
...
# 2. Generate regulatory documentation (if it does not exist)
licit annex-iv
...
## Frequently asked questions from auditors

### How accurate is the AI code detection?
...
The configurable threshold (`confidence_threshold: 0.6`) determines what is counted as AI.

### Are the reports tamper-proof?
