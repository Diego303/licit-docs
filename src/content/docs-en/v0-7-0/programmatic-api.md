...
# Create a default config and modify it
config = LicitConfig()
...
# FRIA (without interactivity — pass responses directly)
fria_gen = FRIAGenerator(context, evidence)
# fria_gen.run_interactive()  # Only works in a terminal
...
4. **structlog** is automatically configured when importing modules. To silence it in scripts, configure it before importing licit.
5. **subprocess calls** (git) have timeouts — they won't hang.
