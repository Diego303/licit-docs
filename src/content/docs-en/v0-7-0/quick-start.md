...
- Generate `.licit.yaml` with a tailored configuration.
...
If you use Architect or Vigil, the connectors enrich compliance evidence:

```bash
licit connect architect    # Reads reports, audit logs, and config from Architect
...
The `trace` command analyzes each commit using 6 heuristics (author, message, volume, co-authors, file patterns, time) and classifies each file as `ai`, `human`, or `mixed`.
...
licit auto-detects answers where possible (models used, guardrails, testing, etc.) and asks for confirmation. It generates `.licit/fria-data.json` and `.licit/fria-report.md`.
...
The report includes an overall summary, a per-framework table, and detail per requirement with evidence and recommendations.
...
Each gap includes a description, actionable recommendation, suggested tools, and effort level.
...
For more detail, see the [full documentation](../introduction/).
