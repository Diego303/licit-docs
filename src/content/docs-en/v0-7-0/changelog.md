...
The changelog system answers the question: **what changed in the AI agent configuration?** It monitors files like `CLAUDE.md`, `.cursorrules`, `AGENTS.md`, and YAML/JSON configs through git history, producing semantic diffs with severity classification.
...
- **Deleted files**: If a file was deleted in a commit, it is recorded as empty content.
...
Parses both versions, then performs recursive dictionary diff:
...
- **OWASP ASI-01** (Excessive Agency): Tracking of changes in guardrails and permissions
...
- Regression tests for 7 bugs found during QA
