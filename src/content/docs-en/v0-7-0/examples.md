...
**Context**: Your team is about to start using Claude Code. You want compliance from day one.
...
licit fria                    # Interactive questionnaire
...
# Or initialize from the root (analyzes the entire monorepo as one project)
cd monorepo/
licit init
licit trace --stats
```
...
# 2. Configure audit log (if architect generates one)
cat >> .licit.yaml << 'EOF'
connectors:
  architect:
    enabled: true
