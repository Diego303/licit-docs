...
System for monitoring changes in AI agent configuration files, with semantic diffing, severity classification, and rendering in Markdown/JSON.
...
The changelog system answers the question: **what changed in the AI agents' configuration?** It monitors files like `CLAUDE.md`, `.cursorrules`, `AGENTS.md`, and YAML/JSON configs through the git history, producing semantic diffs with severity classification.
...
The system is composed of 4 modules in `src/licit/changelog/`:
...
└── renderer.py      # Rendering in Markdown or JSON
```
...
`ConfigWatcher` monitors AI agent configuration files through the git history.
...
| Glob | `.prompts/**/*.md` | Resolved with `Path.glob()` and filters existing files |

### Protections

- **Size guard**: `_MAX_CONTENT_BYTES = 1_048_576` (1 MB) for `git show`. Files larger than this are discarded with a log warning.
...
- **Deleted files**: If a file was deleted in a commit, it is recorded with empty content.

---

## Semantic Differ

`diff_configs()` produces semantic diffs based on the file format.
...
|---|---|---|
| YAML | `.yaml`, `.yml` | Recursive key-value dict with `_diff_dicts()` |
| JSON | `.json` | Recursive key-value dict with `_diff_dicts()` |
| Markdown | `.md` | Section-based by headings with `_parse_md_sections()` |
| Plain text | Other | Full content diff |

### YAML / JSON

Parses both versions, then performs a recursive dictionary diff:
...
Parses ATX headings (`#`, `##`, `###`, etc.) and produces per-section diffs:
...
**Without headings**: If the markdown has no headings, it falls back to full content diff as `(content)`.
...
| **MAJOR** (escalation) | Deletion of a MINOR field | Deleting `guardrails`, deleting `protected_files` |
| **MINOR** | Change in a Markdown section | `section:Rules`, `section:Instructions` |
...
| `output_path` | str | `.licit/changelog.md` | Path to generated changelog |

---

## Integration with Compliance

The agent configuration changelog feeds directly into the `EvidenceBundle`:
