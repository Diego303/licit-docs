...
licit uses a YAML file called `.licit.yaml` in the project root. It is automatically generated with `licit init` but can be manually edited.
...
**Output formats:** `markdown` (default) groups by file and sorts by severity. `json` produces an object `{"changes": [...]}`.
