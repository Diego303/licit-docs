...
If you use a system with multiple Python versions, make sure to use the correct pip:
...
In the root of your project, next to `pyproject.toml` or `package.json`:
...
All 10 commands are functional since v0.7.0 and stabilized in v1.0.0:

| Command | Phase | Current status |
|---|---|---|
| `init` | 1 | **Stable** (v1.0.0 — warning on re-init) |
| `status` | 1+7 | **Stable** (v0.7.0 — connectors + security findings) |
| `connect` | 1+7 | **Stable** (v0.7.0 — availability feedback) |
| `trace` | 2 | **Stable** (v1.0.0 — author date filter, deduplicated store) |
| `changelog` | 3 | **Stable** (v1.0.0 — JSON saves as .json) |
| `fria` | 4 | **Stable** (v1.0.0 — `--auto` non-interactive mode) |
| `annex-iv` | 4 | **Stable** (v0.4.0) |
| `verify` | 4-6 | **Stable** (v0.5.0 — EU AI Act + OWASP) |
| `report` | 6 | **Stable** (v1.0.0 — respects config `output_dir`) |
| `gaps` | 6 | **Stable** (v1.0.0 — clear message without frameworks) |
...
This is necessary because `evidence.py` imports connectors, and connectors need the `EvidenceBundle` type for their signatures.
...
It is not regulation, it is a security best practices framework. However, following OWASP Agentic Top 10 recommendations significantly reduces security risks when using AI agents in development.
...
Yes. licit does not require an internet connection to function. You only need to install dependencies beforehand.

### Is it safe to commit generated reports?

Reports in `.licit/reports/` are generally safe to commit. They contain compliance evaluations, not sensitive data. However, review the content before pushing to a public repository.
...
| **Gap** | Gap between the current state and a compliance requirement |
