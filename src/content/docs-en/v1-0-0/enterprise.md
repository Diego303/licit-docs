...
| Config changelog | Audit trail of agent configuration changes |
...
| **Permissions** | Read-only project access + write to `.licit/` |
...
| Impact assessment (FRIA) | Art. 27 | Interactive generator + `--auto` mode for CI/CD |
...
| Compliance reports | Low (metadata, not code) | Version; share with audit |
...
- **No network**: licit makes no HTTP requests, has no telemetry, no phone-home
