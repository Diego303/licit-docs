...
- **Merge + dedup**: Each `save()` merges with existing records — the most recent per file wins. The store does not grow with repeated executions.
...
- **Error handling**: `PermissionError` and other I/O errors are reported with a clean message
