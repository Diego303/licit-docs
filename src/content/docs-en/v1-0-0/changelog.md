---
title: "Changelog"
description: "AI agent configuration changelog system: watcher, semantic differ, classifier, and renderer."
order: 18
---

# Changelog System

Monitoring system for changes in AI agent configuration files, with semantic diffing, severity classification, and Markdown/JSON rendering.
...
# Change history for all watched files
...
- **Size guard**: `_MAX_CONTENT_BYTES = 1_048_576` (1 MB) in `git show`. Larger files are discarded with a log warning.
- **Timeouts**: 10s for existence verification, 30s for `git log`.
...
Parses both versions, then recursive dict diff:
...
Parses ATX headings (`#`, `##`, `###`, etc.) and produces diffs per section:
...
**No headings**: If the markdown has no headings, it falls back to full content diff as `(content)`.
...
| **MAJOR** (escalation) | Removal of MINOR field | Deleting `guardrails`, deleting `protected_files` |
| **MINOR** | Change in Markdown section | `section:Rules`, `section:Instructions` |
...
3. Sections per file (sorted alphabetically)
4. Within each file: sorted by severity (MAJOR first), then timestamp descending
...
| `output_path` | str | `.licit/changelog.md` | Path for the generated changelog |
