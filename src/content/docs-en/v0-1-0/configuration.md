---
title: "Configuration"
description: "Complete guide for configuring .licit.yaml with all available fields and sections."
order: 4
---

## Configuration File

licit uses a YAML file called `.licit.yaml` at the project root. It is automatically generated with `licit init` but can be edited manually.
...
- `git-infer`: Analyzes heuristics from git history (author patterns, commit messages, change volume).
- `session-log`: Reads session logs from AI agents (Claude Code, Cursor, etc.).
- `git-ai`: Reads annotations from git-ai type tools.
...
Tracks changes in AI agent configuration files through git history.

| Field | Type | Default | Description |
|---|---|---|---|
| `enabled` | bool | `true` | Enable monitoring |
| `watch_files` | list[str] | (see example) | Files to monitor |
| `output_path` | str | `.licit/changelog.md` | Changelog path |
...
Integration with Architect to read audit reports and guardrail configuration.
...
```
.licit/
├── provenance.jsonl    # Traceability store (JSONL)
