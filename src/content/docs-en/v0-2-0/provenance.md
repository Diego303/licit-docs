...
Code traceability system for identifying and documenting what code was written by AI and what by humans.
...
| < 0.5 | `human` — Code likely human-written |
...
`GitAnalyzer` parses the git history and applies the heuristics to each commit.
...
`ProvenanceAttestor` provides HMAC-SHA256 individual signing and batch verification with Merkle tree.
...
To verify integrity of a set of records:
...
3. **Merge**: Combines results from git and sessions (sessions take priority if there is a conflict)
