...
├── frameworks/      Regulatory evaluators (Phase 4 COMPLETED, 5 pending)
...
## Implemented Modules (Phases 1-4)
...
10 commands registered with Click. Eight functional: `init`, `status`, `connect`, `trace`, `changelog`, `fria`, `annex-iv`, `verify`. The remaining (`report`, `gaps`) have full signatures and help text but depend on modules from future phases.
