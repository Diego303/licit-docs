...
The `watch_patterns` are resolved in two ways:

| Type | Example | Resolution |
|---|---|---|
| Exact name | `CLAUDE.md` | Verifies existence in git history (`git log --oneline -1`) |
| Glob | `.prompts/**/*.md` | Resolves with `Path.glob()` and filters existing files |

### Protections

- **Size guard**: `_MAX_CONTENT_BYTES = 1_048_576` (1 MB) in `git show`. Files larger than this are discarded with a log warning.
...
`ensure_ascii=False` for full Unicode support.
