---
title: "Provenance"
description: "Code traceability system: heuristics, git analyzer, JSONL store, attestation, and session readers."
order: 7
---
...
> The configurable threshold `confidence_threshold` (default: 0.6) affects filtering in reports, not the base classification.
...
For each file, the **maximum score** among all commits that modified it is taken. The detection method is always `ProvenanceSource.GIT_INFER`.
...
**Extracted fields:**
- Modified files (from tool calls `Write`, `Edit`)
...
- Odd-numbered records: the last one is duplicated
...
`ProvenanceTracker` orchestrates the complete pipeline:
...
3. **Merge**: Combines results from git and sessions (sessions take priority in case of conflict)
