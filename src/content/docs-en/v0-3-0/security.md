...
- SHA256 is computed for each record, forming the tree leaves
...
- `capture_output=True` — stdout/stderr are captured, not displayed directly.
...
- `subprocess.TimeoutExpired` is caught — returns an empty list without crashing.
- Explicit `check=False` — on all `subprocess.run` calls in provenance and changelog (does not raise an exception on returncode != 0).
