...
class ChangeSeverity(StrEnum):
    ...
    PATCH = "patch"   # Parameter adjustment, formatting
```
...
Snapshot of a configuration file at a point in the git history.
...
All Pydantic models use `model_config = ConfigDict(extra="ignore")` implicitly and allow extra fields in the YAML without error (they are silently ignored).
